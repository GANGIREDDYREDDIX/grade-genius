...
function parseSubjectFromLine(line: string): Subject | null {
  ...
  if (!GRADE_SET.has(grade) || !name || Number.isNaN(creditsNumber) || creditsNumber <= 0 || creditsNumber > 30) {
    return null;
  }
...
async function extractTextFromAspxLike(file: File): Promise<string> {
  ...
  return doc.body?.innerText || doc.documentElement?.textContent || raw;
}
...
export async function extractSemestersFromAspx(file: File): Promise<Semester[]> {
  ...
  const text = await extractTextFromAspxLike(file);
